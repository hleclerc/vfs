...
class ActorSystem:
    ...
    def __init__( self, options: Options ) -> None:
        ...
        self.threads = []
        print( "ActorSystem.py => os.cpu_count()" )
    ...
    def _start_actor( self, actor, args, kwargs, cb, parent_actor ):
        ...
        if actor.is_idempotent():
            ...
            is_cached, dependancy_file, output_args, output_kwargs = self._from_cache( short_name, parameter_repr, actor )
            if is_cached:
                            ...
                            if p[ 1 ] < 0:
                                if os.path.exists( p[ 0 ] ):
                                    if self.options.verbosity() >= 3:
                                        actor.info( f"update of { short_name } because { n } '{ p[ 0 ] }' has appeared" )
