...
void VfsCodegen::get_cpp_flags_in( Vec<Str> &res ) {
    for( const auto &p : extra_cpp_flags )
        res.push_back_unique( p );
    for( const auto &p : inc_paths )
        res.push_back_unique( "-I" + p );
}
...
void VfsCodegen::_add_cpp_flag( const Str &str ) { if ( parent ) return parent->_add_cpp_flag( str ); if ( str.size() ) extra_cpp_flags.push_back_unique( str ); }
void VfsCodegen::_add_inc_path( const Str &str ) { if ( parent ) return parent->_add_inc_path( str ); if ( str.size() ) inc_paths.push_back_unique( str ); }
void VfsCodegen::_add_inc_file( const Str &str ) { if ( parent ) return parent->_add_inc_file( str ); if ( str.size() ) inc_files.push_back_unique( str ); }
void VfsCodegen::_add_lib_flag( const Str &str ) { if ( parent ) return parent->_add_lib_flag( str ); if ( str.size() ) lib_flags.push_back_unique( str ); }
void VfsCodegen::_add_lib_path( const Str &str ) { if ( parent ) return parent->_add_lib_path( str ); if ( str.size() ) lib_paths.push_back_unique( str ); }
void VfsCodegen::_add_lib_name( const Str &str ) { if ( parent ) return parent->_add_lib_name( str ); if ( str.size() ) lib_names.push_back_unique( str ); }
