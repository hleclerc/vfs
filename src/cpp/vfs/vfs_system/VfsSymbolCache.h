...
public:
    ...
    void            register_func          ( const Str &name, const CompilationFlags &compilation_flags, const Str &return_type, const Vec<Str> &arg_types, const Vec<Str> &cast_types, void *symbol );
    SurdefFunc&     add_surdef             ( const Str &file, int line, const Str &name, const Vec<Str> &arg_names = {} );
    void*           find_func              ( const Str &name, const CompilationFlags &compilation_flags, const Str &return_type, const Vec<Str> &arg_types, const Vec<Str> &cast_types );
...
private:
    ...
    void*           load_lib_for           ( const Str &name, const CompilationFlags &compilation_flags, const Str &return_type, const Vec<Str> &arg_types, const Vec<Str> &cast_types );
    Str             cpp_for                ( const Str &name, const CompilationFlags &compilation_flags, const Str &return_type, const Vec<Str> &arg_types, const Vec<Str> &cast_types );
