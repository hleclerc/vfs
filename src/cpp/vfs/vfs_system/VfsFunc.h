...
TT concept HasVfsObjectCtKey = requires ( const T &t ) { vfs_object_ct_key( t ); };
...
#define VFS_CALL( FUNC, RETURN, ... ) \
    vfs_call<#FUNC,RETURN>( __VA_ARGS__ )
...
#define VFS_CALL_METHOD( FUNC, RETURN, ... ) \
    vfs_call<#FUNC "__method",RETURN>( __VA_ARGS__ )

/// if args do not generate any ct_key (vfs_object_ct_key), make a direct call. Else, use a vfs_call
#define VFS_CALL_DINK( FUNC, RETURN, ... ) \
    ( [&]<class... Args>( Args&&...args ) { \
        if constexpr ( ( HasVfsObjectCtKey<Args> || ... ) ) \
            return vfs_call<#FUNC,RETURN>( std::forward<Args>( args )... ); \
        else \
...
#define VFS_CALL_METHOD_DINK( FUNC, RETURN, SELF, ... ) \
    ( [&]<class Self,class... Args>( Self &&self, Args&&...args ) { \
        if constexpr ( HasVfsObjectCtKey<Self> || ( HasVfsObjectCtKey<Args> || ... ) ) \
            return vfs_call<#FUNC "__method",RETURN>( std::forward<Self>( self ), std::forward<Args>( args )... ); \
        else \
