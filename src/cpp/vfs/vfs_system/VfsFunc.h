...
auto vfs_call( CtString<name>, CtStringList<compilations_flags...>, CtIntList<output_indices...>, Args&&... args );
...
#define VFS_CALL_DINK( FUNC, COMPILATION_FLAGS, OUTPUT_INDICES, ... ) \
    ( [&]<class Cf,class Oi,class... Args>( const Cf &compilation_flags, const Oi &output_indices, Args&&...args ) { \
        if constexpr ( IsAVfsWrapper<Cf> || IsAVfsWrapper<Oi> || ( IsAVfsWrapper<Args> || ... ) ) \
            return vfs_call( CtString<#FUNC>(), compilation_flags, output_indices, std::forward<Args>( args )... ); \
        else \
            return FUNC( std::forward<Args>( args )... ); \
    } )( COMPILATION_FLAGS, OUTPUT_INDICES, __VA_ARGS__ )
