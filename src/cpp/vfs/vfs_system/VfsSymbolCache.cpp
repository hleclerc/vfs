...
void VfsSymbolCache::register_func( const Str &name, const CompilationFlags &compilation_flags, const Str &return_type, const Vec<Str> &arg_types, const Vec<Str> &cast_types, void *symbol ) {
    Key key{ name, compilation_flags, return_type, arg_types, cast_types };
...
void *VfsSymbolCache::find_func( const Str &name, const CompilationFlags &compilation_flags, const Str &return_type, const Vec<Str> &arg_types, const Vec<Str> &cast_types ) {
    Key key{ name, compilation_flags, return_type, arg_types, cast_types };
    auto iter = loaded_symbols.find( key );
    if ( iter == loaded_symbols.end() )
        iter = loaded_symbols.insert( iter, { key, load_lib_for( name, compilation_flags, return_type, arg_types, cast_types ) } );
    return iter->second;
}
...
Str VfsSymbolCache::cpp_for( const Str &function_name, const CompilationFlags &compilation_flags, const Str &return_type, const Vec<Str> &arg_types, const Vec<Str> &cast_types ) {
    //
    ...
    auto try_surdef = [&]( const Surdef &surdef ) -> SurdefTrial {
        ...
        for( PI i = 0; i < arg_types.size(); ++i ) {
            // base arg_name
            Str base_name = surdef.arg_name( i );
