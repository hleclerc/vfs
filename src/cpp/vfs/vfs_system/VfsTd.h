...
#define VFS_TD_ATTRIBUTES( NAME, PATH ) public: \
    ...
    static auto          _real_type_call      ( auto &&...args ) { using Result = GET_DT_VALUE( type_promote( CtString<func>(), DECAYED_CT_OF( args )... ) ); return vfs_call<func,Result>( FORWARD( args )... ); }; \
    \
