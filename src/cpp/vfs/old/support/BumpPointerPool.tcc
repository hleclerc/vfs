...
inline std::pair<char *,std::size_t> BumpPointerPool::allocate_max( std::size_t min_size, std::size_t max_size, std::size_t alig ) {
    char *res_ptr = allocate( min_size, alig );
    std::size_t res_len = min_size;
    ...
    // check max_size
    if ( res_len > max_size ) {
        std::size_t delta = res_len - max_size;
...
inline char *BumpPointerPool::allocate( std::size_t size, std::size_t alig ) {
    using std::malloc;
    using std::max;
    ...
    if ( current_ptr.cp > ending_ptr ) {
        std::size_t frame_size = max( 4096ul, sizeof( Frame * ) + sizeof( char * ) + alig - 1 + size );
...
inline char *BumpPointerPool::allocate( std::size_t size ) {
    using std::malloc;
    using std::max;
    ...
    current_ptr.cp += size;
    if ( current_ptr.cp > ending_ptr ) {
        std::size_t frame_size = max( 4096ul, sizeof( Frame * ) + sizeof( char * ) + size );
        Frame *new_frame = new ( malloc( frame_size ) ) Frame;
        new_frame->ending_ptr = reinterpret_cast<char *>( new_frame ) + frame_size;
        new_frame->prev_frame = last_frame;
        last_frame = new_frame;
