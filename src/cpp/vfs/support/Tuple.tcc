...
DTP TTY void UTP::filtered_apply_seq( const auto &func ) const {
    if constexpr ( Y<Head>::value )
        func( head );
    tail.template filtered_apply_seq<Y>( func );
}
...
DTP TTY auto UTP::filtered_apply( auto &&func, auto &&...end_args ) const {
    if constexpr ( Y<Head>::value )
        return tail.template filtered_apply<Y>( FORWARD( func ), FORWARD( end_args )..., head );
    else
        return tail.template filtered_apply<Y>( FORWARD( func ), FORWARD( end_args )... );
}
